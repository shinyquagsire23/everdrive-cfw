...
u8 diskInit() {
    ...
    //if (GBARAM_PREV_CARDTYPE != 0xFF && GBARAM_PREV_CARDTYPE != 0x0) goto fast_init;

    bi_sd_speed(SD_SPD_LO);

    bi_sd_mode(SD_MODE8);

    for (i = 0; i < 40; i++)bi_sd_dat_wr(0xff);
...
fast_init:
    card_type = GBARAM_PREV_CARDTYPE;
    bi_sd_mode(SD_MODE8);
    bi_sd_speed(SD_SPD_HI);
    disk_addr = ~0;

    if (GBARAM_PREV_CARDTYPE & 0x8000)
    {
        disk_addr = 0;
        diskCloseRW();
    }

    GBARAM_PREV_CARDTYPE = card_type | 0x8000;
...
u8 diskOpenRead(u32 saddr) {
    ...
    u8 resp;
    if ((card_type & SD_HC) == 0)saddr *= 512;
...
u8 diskWrite(u32 sd_addr, void *src, u16 slen) {
    ...
    while (slen--) {
        ...
        if (i == 0)return DISK_ERR_WR3; //timeout
        resp = 0;
